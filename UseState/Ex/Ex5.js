...
export default function Ex5TodoList({ navigation }) { 
  // ===== STATE เติมเอง ===== 
  // (1) ประกาศ state ชื่อ text ชนิดข้อความ เริ่มต้น '' 
  const [text, setText] = useState('');
  
  // (2) ประกาศ state ชื่อ todos ชนิดอาเรย์ เริ่มต้น [] 
  const [todos, setTodos] = useState([]); 

  // (3) addTodo: เพิ่มงานใหม่โดยไม่แก้อาเรย์เดิม ([...todos, item]) แล้วล้างช่อง text 
  const addTodo = () => { 
    if (text.trim() === '') return; 
    const newTodo = {
  ...
  // (5) toggleDone(id): สลับ done ของ item ที่ตรง id ด้วย todos.map(...) อย่าลืม( spread ...t) 
  const toggleDone = (id) => { 
  ...
  return ( 
    <View style={s.container}> 
      ...
      {/* (7) แสดงจำนวนทั้งหมด และจำนวนที่ done === true */} 
      <Text style={s.counter}>
        ทั้งหมด {todos.length} รายการ เสร็จแล้ว {todos.filter(t => t.done).length}
      </Text> 
      ...
      <FlatList 
        ...
        ListEmptyComponent={<Text style={s.empty}>- ลองเพิ่มดูสิ</Text>}/> 
        <Button title='Go to Ex6' 
