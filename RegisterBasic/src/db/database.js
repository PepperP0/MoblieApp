...
export async function findDuplicate(db, student_id, username) {
  ...
  if (!row) return null
  if (row.student_id === student_id) return 'studentId'
  return 'username'
}

export async function registerStudent(db, { name, surname, student_id, username, password }) {
  const duplicate = await findDuplicate(db, student_id, username)
  ...
  if (duplicate === 'username') {
    return { ok: false, field: 'username', message: 'ชื่อผู้ใช้นี้นี้ลงทะเบียนไปแล้ว' }
  }
  ...
  try {
    const result = await db.runAsync(
      `INSERT INTO students (name, surname, student_id, username, password_salt, password_hash, create_at)
       VALUES (?, ?, ?, ?, ?, ?, ?)`,
      [name, surname, student_id, username, salt, hash, new Date().toISOString()]
    )
    return { ok: true, id: result.lastInsertRowId }
  } catch (e) {
    console.warn('registerStudents ล้มเหลว', e)
    return { ok: false, field: null, message: 'บันทึกไม่สำเร็จกรุณาลองใหม่' }
  }
}
